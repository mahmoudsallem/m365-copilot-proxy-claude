...
describe("Anthropic Messages compatibility", () => {
  ...
  it("combines legacy system-role messages with the top-level system prompt", () => {
    const body = AnthropicMessagesRequest.parse({
      model: "claude-sonnet",
      system: "Top-level policy",
      messages: [
        { role: "system", content: [{ type: "text", text: "Legacy client policy" }] },
        { role: "user", content: "Hello" },
      ],
    });

    expect(toOpenAIChatRequest(body).messages).toEqual([
